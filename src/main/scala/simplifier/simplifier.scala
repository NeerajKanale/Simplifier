...
object Simplifier {
  ...
  def simplifyDeadAssignment(list: List[Node]): List[Node] = {
    if (list.length < 2) list
      ...
      val simplifiedList = for (index <- 1 until list.length; if ((list(index - 1), list(index)) match {
        case (Assignment(x, y), Assignment(x2, y2)) if x == x2 && x2 != y2 => false
        case _ => true
      }))
        yield list(index - 1)
  ...
  def simplify(node: Node): Node = node match {
    case nodeList@NodeList(list) =>
      ...
      if (simplifiedList.isEmpty) EmptyNode()
      else if (simplifiedList.length == 1) simplifiedList.head
        ...
        (_left, _right) match {
          ...
          case _ => val __left = if (_left.toStr < _right.toStr) _left else _right
            ...
            (__left, __right) match {
              ...
              case (BinExpr("+", BinExpr("*", v1, y1), interExpression), BinExpr("*", v2, z2)) if v1 == v2 =>
                simplifyBinExpr(BinExpr("+", interExpression, BinExpr("*", v1, BinExpr("+", y1, z2))))
