...
class parserSpec extends Specification {
  ...
  "simplifier" should {
    ...
    "recognize tuples" in {
     // parseString("x=()") must not(throwA[IllegalArgumentException])
      parseString("x=(a, b)") must not(throwA[IllegalArgumentException])
      parseString("x=(a,)") must not(throwA[IllegalArgumentException])
